 ...
 * Updated with your specific sounds: jump.wav and boom.wav
 */
...
// --- 1. AUDIO SYSTEM (Updated for jump.wav and boom.wav) ---
const sfx = {
    jump: new Audio('/static/audio/jump.wav'),
    phase: new Audio('/static/audio/jump.wav'), // Reusing jump for phase until you get a 3rd sound
    crash: new Audio('/static/audio/boom.wav'), // Mapped to your boom.wav
    bgm: new Audio('/static/audio/music.wav')   // If you have music, name it music.wav
};

// Audio Settings
sfx.jump.volume = 0.3;
sfx.phase.volume = 0.2; // Lower volume for phase so it sounds different
    ...
    update(h) {
        ...
        if (this.phaseCooldown > 0) {
            ...
            if (this.color === '#00f0ff') {
                ...
                document.getElementById('phase-cooldown-bar').style.width = percent + "%";
            }
        }
        if (state.active && state.score % 2 === 0) System.spawnPart(this.x, this.y + this.h/2, this.color, 1);
    }
    draw(ctx) {
        ctx.save();
        ...
        ctx.globalAlpha = this.phasing ? 0.3 : 1;
        ctx.beginPath();
        if(this.g === 1) { 
...
const System = {
    boot: () => {
        ...
        sfx.bgm.play().catch(() => console.log("Music waiting for interaction"));
    ...
    spawnPart: (x, y, color, count) => {
        for(let i=0; i<count; i++) {
            state.particles.push({
                x, y, color, vx: (Math.random()-0.5)*8, vy: (Math.random()-0.5)*8, life: 1.0
            });
        }
    },
    ...
    update: () => {
        ...
        if(state.pA.phasing) { bar.innerText = "PHASING"; bar.style.color = "#fff"; }
        else if(state.pA.phaseCooldown > 0) { bar.innerText = "WAIT"; bar.style.color = "#444"; }
        else { bar.innerText = "READY"; bar.style.color = "#00f0ff"; }
    ...
    draw: () => {
        [ctxA, ctxO].forEach((ctx, i) => {
            ...
            if(id === 'A') {
                state.pA.draw(ctx);
                state.obsA.forEach(o => { ctx.fillStyle = o.glitch ? '#fff':'#ff003c'; ctx.fillRect(o.x, o.y, o.w, o.h); });
            } else {
                state.pO.draw(ctx);
                state.obsO.forEach(o => { ctx.fillStyle = o.glitch ? '#fff':'#ff003c'; ctx.fillRect(o.x, o.y, o.w, o.h); });
            }
        });
    },
...
window.onkeydown = (e) => {
    ...
    if(e.code === 'ShiftLeft' || e.code === 'ShiftRight') {
        if(state.pA.phaseCooldown <= 0) {
            ...
            // Play Phase Sound (using jump.wav for now)
