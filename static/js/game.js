/**
 * NEON PARADOX: MASTER EDITION (V13.0)
...
class Shard {
    ...
    update() {
        ...
        this.angle += this.vr; this.life -= 0.02;
    }
    draw(ctx) {
        ctx.save(); ctx.translate(this.x, this.y); ctx.rotate(this.angle);
        ctx.globalAlpha = this.life; ctx.fillStyle = this.color;
        ctx.beginPath(); ctx.moveTo(0, -this.size/2); ctx.lineTo(this.size/2, this.size/2); ctx.lineTo(-this.size/2, this.size/2); ctx.closePath();
...
const System = {
    ...
    update: () => {
        ...
        // --- SPEED FIX ---
        // Increments much slower now. Difficulty scales based on score.
        state.score += 0.15; 
        state.speed = 7 + (Math.floor(state.score) / 150); // Balanced speed scaling
        ...
        [ {p: state.pA, obs: state.obsA, id: 'A'}, {p: state.pO, obs: state.obsO, id: 'O'} ].forEach(cfg => {
            ...
            cfg.obs.forEach((o, i) => {
                o.x -= state.speed;
                if(!cfg.p.phasing && cfg.p.x + 8 < o.x + o.w && cfg.p.x + cfg.p.w - 8 > o.x && cfg.p.y + 8 < o.y + o.h && cfg.p.y + cfg.p.h - 8 > o.y) {
                    System.die(cfg.p);
                }
    ...
    gameLoop: () => { System.update(); System.draw(); if(state.active || state.shards.length > 0) requestAnimationFrame(System.gameLoop); },
    submit: async () => {
    ...
    showManual: () => alert("SPACE: Jump | W/S: Flip Gravity | SHIFT: Phase")
};
...
class Player {
    ...
    update(h) {
        ...
        if (this.y > h - this.h || this.y <= 0) { this.angle = 0; this.v = 0; if(this.y > h - this.h) this.y = h - this.h; else this.y = 0; }
    ...
    draw(ctx) {
        ...
        ctx.shadowBlur = this.phasing ? 50 : 20; ctx.shadowColor = this.color; ctx.fillStyle = this.phasing ? "#fff" : this.color; ctx.globalAlpha = this.phasing ? 0.4 : 1;
        ctx.beginPath();
...
// Preview Engine Logic (Simplified)
const PreviewEngine = {
    frame: 0, obsA: [], obsO: [],
