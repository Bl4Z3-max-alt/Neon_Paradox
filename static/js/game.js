...
let state = {
    ...
    lastTime: 0, selectedColor: '#00f0ff', splitTime: 30,
...
class Player {
    constructor(color) { this.w = 35; this.h = 35; this.x = 80; this.y = 100; this.v = 0; this.g = 1; this.color = color; this.phasing = false; this.phaseCooldown = 0; this.angle = 0; }
    update(h, dt) {
    ...
    draw(ctx) {
        ...
        ctx.globalAlpha = this.phasing ? 0.4 : 1.0; // Reset alpha check
        ctx.shadowBlur = (state.glitchActive) ? 40 : 20; 
        ...
        ctx.fill();
        ctx.restore();
        ctx.globalAlpha = 1.0; ctx.shadowBlur = 0; // HARD RESET
    }
}
...
const System = {
    ...
    loadWithBar: (target) => {
        ...
        let interval = setInterval(() => {
            ...
            if(progress >= 100) { clearInterval(interval); screen.classList.remove('active'); 
            if(target === 'HUB') { System.switchRoom('hub-room'); PreviewEngine.loop(); System.loadScores(); } else if(target === 'VOID') System.boot(); }
        }, 50);
    },
    ...
    update: (dt) => {
        ...
        state.score += 0.1 * cDt; // Slower score
    ...
    submit: async () => {
        ...
        location.reload(); // Reset for mobile memory management
    },
    loadScores: async () => { const r = await fetch('/api/leaderboard'); const data = await r.json(); document.getElementById('score-list').innerHTML = data.map((s, i) => `<div style="display:flex; justify-content:space-between; padding:5px; border-bottom:1px solid #222;"><span>${i+1}. ${s.name}</span><span>${s.score}</span></div>`).join(''); }
};
