...
let state = {
    ...
    lastTime: 0, selectedColor: '#00f0ff', timeScale: 1
};
...
const System = {
    ...
    boot: () => {
        ...
        state.active = true; state.startTime = Date.now(); state.score = 0; state.speed = 7; state.isSplit = false; state.timeScale = 1;
    ...
    update: (dt) => {
        ...
        if(state.timeScale < 1) state.timeScale += 0.002 * dt;
        ...
        [ {p: state.pA, obs: state.obsA, id: 'A'}, {p: state.pO, obs: state.obsO, id: 'O'} ].forEach(cfg => {
            ...
            if(Math.random() < 0.005 * dt) state.pickups.push({x: canvasA.width + 50, y: Math.random()*(canvasA.height-30), w: 20, h:20, id: cfg.id});
        ...
        state.pickups.forEach((pk, i) => {
            ...
            if(player.x < pk.x + pk.w && player.x + player.w > pk.x && player.y < pk.y + pk.h && player.y + player.h > pk.y) {
                state.score += 50; state.timeScale = 0.5; state.pickups.splice(i, 1);
            }
    ...
    gameLoop: (currentTime) => { 
        const dt = (currentTime - state.lastTime) / 16.67; state.lastTime = currentTime; 
        System.update(Math.min(dt, 2)); System.draw(); requestAnimationFrame(System.gameLoop); 
    },
    submit: async () => {
        ...
        System.switchRoom('hub-room'); document.getElementById('death-overlay').style.display = 'none'; sfx.lobby.play().catch(() => {}); System.loadWithBar('HUB');
    },
