...
let state = {
    ...
    lastTime: performance.now(), selectedColor: '#00f0ff', splitTime: 30,
...
class Player {
    constructor(color) { this.w = 35; this.h = 35; this.x = 80; this.y = 150; this.v = 0; this.g = 1; this.color = color; this.phasing = false; this.phaseCooldown = 0; this.angle = 0; }
    update(h, dt) {
    ...
    draw(ctx) {
        ...
        ctx.globalAlpha = this.phasing ? 0.4 : 1.0; ctx.shadowBlur = state.glitchActive ? 40 : 15;
        ctx.shadowColor = this.color; ctx.fillStyle = (this.phasing || state.glitchActive) ? "#fff" : this.color;
        ctx.beginPath();
...
const System = {
    initTerminal: () => { sfx.lobby.play().catch(()=>{}); System.switchRoom('room-hub'); PreviewEngine.loop(); System.loadScores(); },
    switchRoom: (id) => { 
    ...
    loadScores: async () => { const r = await fetch('/api/leaderboard'); const data = await r.json(); document.getElementById('score-list').innerHTML = data.map((s, i) => `<div style="display:flex; justify-content:space-between; padding:8px; border-bottom:1px solid #222;"><span>${i+1}. ${s.name}</span><span>${s.score}</span></div>`).join(''); },
    
    loadWithBar: (target) => {
        const screen = document.getElementById('loading-screen'), bar = document.getElementById('progress-bar');
        System.switchRoom('loading-screen');
        let progress = 0;
        let interval = setInterval(() => {
            progress += 5; bar.style.width = progress + "%";
            if(progress >= 100) { clearInterval(interval); if(target === 'VOID') System.boot(); }
        }, 50);
    },

    boot: () => {
        sfx.lobby.pause(); System.switchRoom('room-void');
    ...
    update: (dt) => {
        ...
        state.score += 0.1 * cDt; state.speed = 7 + (state.score / 300);
    ...
    draw: () => {
        [ctxA, ctxO].forEach((ctx, i) => {
            ...
            ctx.fillStyle = '#fff'; state.pickups.filter(p => p.id === id).forEach(p => ctx.fillRect(p.x, p.y, p.w, p.h));
            if(state.active) {
    ...
    blueScreen: () => { 
        state.active = false; document.getElementById('room-bsod').style.display = 'flex'; 
        setTimeout(() => { document.getElementById('room-bsod').style.display = 'none'; System.switchRoom('room-hub'); sfx.lobby.play(); }, 3000); 
    },

    gameLoop: (t) => { let dt = (t - state.lastTime)/16.67; state.lastTime = t; System.update(Math.min(dt, 2)); System.draw(); if(state.active || state.shards.length > 0) requestAnimationFrame(System.gameLoop); },
    ...
    submit: async () => {
        ...
        sfx.lobby.play(); System.switchRoom('room-hub'); System.loadScores();
    }
};

const Actions = {
    jump: () => { if(!state.active) return; state.pA.v = (state.pA.g === 1) ? -14 : 14; if(state.isSplit) state.pO.v = (state.pO.g === 1) ? -14 : 14; sfx.jump.play(); },
...
window.onkeydown = (e) => { if(e.code === 'Space') Actions.jump(); if(e.code === 'KeyW') Actions.flip(-1); if(e.code === 'KeyS') Actions.flip(1); if(e.code === 'ShiftLeft') Actions.phase(); };
...
const PreviewEngine = {
    ...
    draw: (ctx, color, obsArr) => {
        ...
        ctx.globalAlpha = 0.1; ctx.strokeStyle = color; for(let x=-(PreviewEngine.frame%50); x<prevO.width; x+=50) ctx.strokeRect(x,0,50,prevO.height);
        ctx.globalAlpha = 1; ctx.fillStyle = color; ctx.fillRect(30, 50 + Math.sin(PreviewEngine.frame*0.1)*30, 20, 20);
        ctx.fillStyle = '#ff003c'; if(PreviewEngine.frame%60===0) obsArr.push({x: prevO.width, y: Math.random()*80, w:10, h:30});
        obsArr.forEach((o, i) => { o.x -= 4; ctx.fillRect(o.x, o.y, o.w, o.h); if(o.x < -20) obsArr.splice(i, 1); });
    },
    loop: () => { if(state.active) return; PreviewEngine.frame++; PreviewEngine.draw(pCtxA, '#00f0ff', []); PreviewEngine.draw(pCtxO, '#ff8c00', PreviewEngine.obsO); requestAnimationFrame(PreviewEngine.loop); }
};