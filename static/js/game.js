...
class Player {
    ...
    draw(ctx) {
        ...
        ctx.shadowBlur = (state.phantomMode > 0) ? 60 : 20; ctx.shadowColor = (state.phantomMode > 0) ? '#fff' : this.color;
        ctx.fillStyle = this.phasing ? "#fff" : (state.phantomMode > 0 ? '#fff' : this.color);
...
const System = {
    ...
    updateSettings: () => { state.splitTime = document.getElementById('set-split-timer').value; document.getElementById('split-timer-val').innerText = state.splitTime + "s"; },
    ...
    boot: () => {
        ...
        state.active = true; state.startTime = Date.now(); state.score = 0; state.speed = 7; state.isSplit = false; state.phantomMode = 0;
    ...
    drawCity: (ctx) => {
        ...
        for(let i=0; i<5; i++) {
            let x = (i * 400) - (state.score * 2) % 400;
            ctx.fillRect(x, 100, 150, canvasA.height); ctx.fillRect(x+200, 50, 100, canvasA.height);
        }
        ctx.globalAlpha = 1;
    },
    ...
    update: (dt) => {
        ...
        state.score += 0.15 * dt; state.speed = Math.min(7 + (state.score / 250), 18);
        ...
        if(state.phantomMode > 0) {
            state.phantomMode -= 1 * dt;
            document.getElementById('overdrive-status').style.display = 'block';
        } else { document.getElementById('overdrive-status').style.display = 'none'; }
        ...
        [ {p: state.pA, obs: state.obsA, id: 'A'}, {p: state.pO, obs: state.obsO, id: 'O'} ].forEach(cfg => {
            ...
            // --- RARE PICKUP SPAWN (0.1% chance) ---
            if(Math.random() < 0.001 * dt) state.pickups.push({x: canvasA.width + 50, y: Math.random()*(canvasA.height-30), w: 30, h:30, id: cfg.id, glow: 0});
            ...
            cfg.obs.forEach((o, i) => {
                ...
                if(cfg.p.x < o.x + o.w && cfg.p.x + cfg.p.w > o.x && cfg.p.y < o.y + o.h && cfg.p.y + cfg.p.h > o.y) {
                    if(state.phantomMode > 0) { System.spawnPart(o.x, o.y, '#fff', 10); cfg.obs.splice(i, 1); }
        ...
        state.pickups.forEach((pk, i) => {
            pk.x -= state.speed * dt; pk.glow += 0.1 * dt;
            ...
            if(player.x < pk.x + pk.w && player.x + player.w > pk.x && player.y < pk.y + pk.h && player.y + player.h > pk.y) {
                state.phantomMode = 360; state.score += 250; state.pickups.splice(i, 1);
            }
    ...
    draw: () => {
        [ctxA, ctxO].forEach((ctx, i) => {
            ...
            // Draw Rare Pickups with pulsing glow
            state.pickups.filter(p => p.id === id).forEach(p => {
                ctx.fillStyle = '#fff'; ctx.shadowBlur = 10 + Math.sin(p.glow)*10; ctx.shadowColor = '#ffd700';
    ...
    die: (p) => {
        state.active = false; if(!state.muted) { try { sfx.crash.currentTime = 0; sfx.crash.play(); } catch(e){} } System.shake();
...
const Actions = {
    jump: () => { if(!state.active) return; state.pA.v = (state.pA.g === 1) ? -14 : 14; if(state.isSplit) state.pO.v = (state.pO.g === 1) ? -14 : 14; if(!state.muted) { try { sfx.jump.currentTime = 0; sfx.jump.play(); } catch(e){} } },
